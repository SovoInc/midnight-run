...
async fn post_alias(db: web::Data<Db>, body: web::Json<AliasRequest>) -> HttpResponse {
    match db.upsert_alias(&body.alias) {
        Ok((id, alias, wallet)) => HttpResponse::Ok().json(Player {
            id, alias, wallet_address: wallet, created_at: String::new(),
        }),
...
async fn get_top_scores(db: web::Data<Db>, query: web::Query<LimitQuery>) -> HttpResponse {
    ...
    match db.top_scores(limit) {
        Ok(rows) => {
            let entries: Vec<ScoreEntry> = rows.into_iter().enumerate().map(|(i, (_, alias, score, dist, pid))| {
                ScoreEntry { rank: (i + 1) as i64, alias, score, distance: dist, player_id: pid }
            }).collect();
