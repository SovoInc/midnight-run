...
impl Db {
    ...
    pub fn channel_leaderboard(
        ...
        let total_score: f64 = conn.query_row(
            &format!("SELECT COALESCE(SUM(best), 0) FROM (SELECT MAX(s.score) as best FROM scores s JOIN players p ON s.player_id = p.id WHERE s.created_at >= ?1 AND s.created_at <= ?2 AND p.network_id = ?3 GROUP BY s.player_id {})", ach_having_4),
            params![start, end, network_id, min_ach], |r| r.get(0),
        )?;
        let mut stmt = conn.prepare(
            &format!("SELECT COALESCE(p.wallet_address, 'alias:' || p.alias), COALESCE(p.wallet_address, p.alias), MAX(s.score) as best
             FROM scores s JOIN players p ON s.player_id = p.id
             WHERE s.created_at >= ?1 AND s.created_at <= ?2 AND p.network_id = ?5
             GROUP BY s.player_id {} ORDER BY best DESC LIMIT ?3 OFFSET ?4", ach_having_6),
        )?;
    ...
    pub fn channel_cumulative(
        ...
        let total_score: f64 = conn.query_row(&q_total_score, params![start, end, network_id, min_ach], |r| r.get(0))?;
    ...
    pub fn channel_interactions(
        ...
        let mut stmt = conn.prepare(
            &format!("SELECT COALESCE(p.wallet_address, 'alias:' || p.alias), COALESCE(p.wallet_address, p.alias), CAST(COUNT(*) AS REAL) as runs
             FROM scores s JOIN players p ON s.player_id = p.id
             WHERE s.created_at >= ?1 AND s.created_at <= ?2 AND p.network_id = ?5
             GROUP BY s.player_id {} ORDER BY runs DESC LIMIT ?3 OFFSET ?4", ach_having_6),
        )?;
        let rows = stmt.query_map(params![start, end, limit, offset, network_id, min_ach], |row| {
            Ok((row.get::<_, String>(0)?, row.get::<_, Option<String>>(1)?, row.get::<_, f64>(2)?))
        })?.collect::<Result<Vec<_>>>()?;
